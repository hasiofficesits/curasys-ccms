                    ...
                    <div class="row g-2">
                        ...
                        {{-- <div class="col-md-4 mt-3 d-grid text-right">
                            <button id="btn_update"
                                class="btn rounded-pill btn-md btn-warning waves-effect waves-light">Update</button>
                        </div> --}}
                        <div class="col-md-6 mt-3 d-grid text-right">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" 
                        onclick="window.location.href='{{ route('load_due_inv_page') }}'">
                        Close
                    </button>
...
@section('script')
    ...
    <script>
        ...
        function load_debtor_accounts() {
            $.ajax({
                "method": "GET",
                "url": "{{ route('ajax_get_debotor_accs') }}",
        ...
        function load_cash_ledgers_to_select_box() {
            $.ajax({
                "method": "GET",
                "url": "{{ route('ajax_get_cash_ledgers') }}",
                "success": function(response) {
        ...
        //     @foreach ($app_body as $value)
        //         @if ($value->Type != 'TblStock_Pharma')
                    
        //             ds.push({
        //                 "ID": '{{ $value->ID }}',
        //                 "Name": "{{ trim($value->service->Name) }}",
        ...
        //         @else
        //             @if ($value->Qty_taken != null)
        //                 ds.push({
        //                     "ID": '{{ $value->ID }}',
        //                     "Name": "{{ trim($value->item->Brand_Name) }}",
        //                     "Rate": '{{ $value->Unit_Price }}',
        //                     "Qty": '{{ $value->Qty_taken }}',
        //                     "Total": '{{ $value->Total }}',
        //                     "status": "exist_data",
        ...
        function pay_invoice() {
            ...
            if (pay_amt > paid_amt) {
                ...
                $.ajax({
                    url: "{{ route('pay_invoice_appointment') }}",
                    method: 'POST',
                    data: data,
                    success: function(response) {
        ...
        function print_invoice() {
            ...
            setTimeout(function() {
                window.location.href = "{{ route('load_due_inv_page') }}"; 
            }, 1000);
        }
        ...
        function view_pres() {
            ...
            $.post('{{ route('load_taken_prescription') }}', {
                    "app_id": app_id
                },
                function(response) {
                    otable_pres.clear();
